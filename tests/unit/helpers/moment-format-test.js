...
module('moment-format', function(hooks) {
  ...
  test('can inline timeZone (New York)', async function(assert) {
    ...
    this.set('date', 0);
    await render(hbs`{{moment-format date 'LLLL' timeZone='America/New_York'}}`);
    assert.equal(this.$().text(), 'Wednesday, December 31, 1969 7:00 PM');
  });
  ...
  test('can inline timeZone (Los Angeles)', async function(assert) {
    ...
    this.set('date', 0);
    await render(hbs`{{moment-format date 'LLLL' timeZone='America/Los_Angeles'}}`);
  ...
  test('can be called using subexpression', async function(assert) {
    ...
    this.owner.register('helper:get-format', helper(function() {
      return 'L';
    }));
